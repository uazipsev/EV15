...
void UART2_init(void) {
    ...
    UART2_buff_init(&input_buffer_2);
    UART2_buff_init(&output_buffer_2);
...
unsigned char UART2_buff_get(struct UART_ring_buff_2* _this) {
    unsigned char c;
    if (_this->count > 0) {
...
void UART2_buff_flush(struct UART_ring_buff_2* _this, const int clearBuffer) {
    _this->count = 0;
    _this->head = 0;
    _this->tail = 0;
    if (clearBuffer) {
...
int UART2_buff_size(struct UART_ring_buff_2* _this) {
    return (_this->count);
}
...
unsigned char UART2_buff_peek(struct UART_ring_buff_2* _this) {
    return _this->buf[_this->tail];
}


int Receive2_available(void) {
    return UART2_buff_size(&input_buffer_2);
}

unsigned char Receive2_get(void) {
    return UART2_buff_get(&input_buffer_2);
}

void Send2_put(unsigned char _data) {
    UART2_buff_put(&output_buffer_2, _data);
    if (Transmit_stall_2 == true) {
        Transmit_stall_2 = false;
        U2TXREG = UART2_buff_get(&output_buffer_2);
    }
}
...
void __attribute__((interrupt, no_auto_psv)) _U2RXInterrupt(void) {
    ...
    UART2_buff_put(&input_buffer_2, data);
    IFS1bits.U2RXIF = 0; // Clear RX interrupt flag
    IFS1bits.U2RXIF = 0; // Clear RX interrupt flag
}
...
void __attribute__((interrupt, no_auto_psv)) _U2TXInterrupt(void) {
    //LED ^= 1;
    if (UART2_buff_size(&output_buffer_2) > 0) {
        U2TXREG = UART2_buff_get(&output_buffer_2);
    } else {
